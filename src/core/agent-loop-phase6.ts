/**
 * Agent Loop Phase 6 — Polish & Extend
 *
 * Adds error recovery (retry, fallback, logging),
 * extension points (plugins, hooks, agent definitions, MCP),
 * and max-turns safety.
 *
 * Run: npm run phase6
 */
...
import {
  executeWithFallback,
  getFailureCounts,
} from "../resilience/tool-fallback.js";
import { HookManager } from "../extensions/hooks.js";
import { loadPlugins } from "../extensions/plugin-loader.js";
import { loadAgentDefinitions } from "../extensions/agent-definitions.js";
import { MCPServer } from "../extensions/mcp-server.js";
import type { AgentDefinition } from "../extensions/types.js";
...
async function initContext(): Promise<void> {
  ...
  const pluginDir = path.join(cwd, ".angel-agent", "plugins");
  const pluginTools = await loadPlugins(pluginDir);
  for (const tool of pluginTools) {
    registry.register(tool);
    mcpServer.registerTool(tool);
  }
  pluginCount = pluginTools.length;

  agentDefinitions = await loadAgentDefinitions(cwd);
  ...
  logger.info("Context initialized", {
    ...
    git: projectContext.gitBranch ?? "none",
    tokens: promptTokens,
    plugins: pluginCount,
    agentDefs: agentDefinitions.length,
  });
  ...
  if (pluginCount > 0) {
    console.log(dim(`  [context] Plugins: ${pluginCount} tools loaded`));
  }
  if (agentDefinitions.length > 0) {
    console.log(
      dim(`  [context] Agent defs: ${agentDefinitions.length} loaded`)
    );
  }
}
...
async function executeTool(
  ...
  const preResult = await hookManager.run("pre_tool", {
    toolName: name,
    input,
  });
  if (!preResult.proceed) {
    logger.info(`Hook blocked tool: ${name}`);
    return { result: "Tool execution blocked by hook", isError: true };
  }
  const effectiveInput = preResult.modifiedInput ?? input;

  const decision = await permissionManager.checkPermission(tool, effectiveInput);
  if (decision === "deny") {
    return { result: "Permission denied by user", isError: true };
  }

  const fallbackResult = await executeWithFallback(tool, effectiveInput, {
    timeout: 30000,
    retries: 1,
  });

  const postResult = await hookManager.run("post_tool", {
    toolName: name,
  ...
  const finalResult = postResult.modifiedResult ?? fallbackResult.result;

  if (fallbackResult.fallbackUsed) {
    logger.warn(`Tool fallback used for ${name}`, {
      originalError: fallbackResult.originalError,
    });
  }

  return {
    result: contextManager.truncateFileContent(finalResult),
    isError: fallbackResult.isError,
  };
}
...
function checkTurnLimit(): boolean {
  ...
  if (ratio >= 1) {
    console.log(
      red(
        `\n  [safety] Max turns reached (${turnCount}/${maxTurns}). Stopping.`
      )
    );
    logger.error("Max turns reached", { turnCount, maxTurns });
    return false;
  }

  if (ratio >= TURN_WARN_THRESHOLD && turnCount % 5 === 0) {
    console.log(
      yellow(
        `  [safety] Turn ${turnCount}/${maxTurns} (${Math.round(ratio * 100)}%)`
      )
    );
    logger.warn("Approaching turn limit", { turnCount, maxTurns });
  }
...
async function chatStreaming(userMessage: string): Promise<string> {
  ...
  while (true) {
    ...
    try {
      await hookManager.run("pre_api", { messages });
      ...
      await hookManager.run("post_api", { messages });
...
function handlePlugins(): void {
  ...
  const builtinNames = new Set([
    "read_file", "list_directory", "write_file", "edit_file", "run_command",
  ]);
  const pluginTools = allTools.filter((t) => !builtinNames.has(t.name));
  ...
  for (const tool of pluginTools) {
    console.log(dim(`  ${tool.name} (${tool.riskLevel}) — ${tool.description}`));
  }
}
...
function handleHooks(): void {
  ...
  if (hooks.length === 0) {
    console.log(dim("  No hooks registered."));
    return;
  }

  for (const hook of hooks) {
    console.log(
      dim(`  ${hook.name} [${hook.type}] priority=${hook.priority ?? 100}`)
    );
  }
}

function handleLogs(args: string): void {
  const count = parseInt(args.trim(), 10) || 20;
    ...
    const levelColor =
      entry.level === "error" ? red
        : entry.level === "warn" ? yellow
          : entry.level === "debug" ? dim
            : cyan;
    const dataStr = entry.data ? ` ${JSON.stringify(entry.data)}` : "";
    console.log(
      dim(`  ${time} `) + levelColor(`[${entry.level}]`) +
        dim(` ${entry.module}: ${entry.message}${dataStr}`)
    );
  }
}

function handleTurns(): void {
  const ratio = turnCount / maxTurns;
  const color = ratio > TURN_WARN_THRESHOLD ? yellow : dim;
  console.log(
    color(`  Turns: ${turnCount}/${maxTurns} (${Math.round(ratio * 100)}%)`)
  );

  const failures = getFailureCounts();
  if (failures.size > 0) {
    console.log(dim("  Tool failures:"));
...
function handleAgentDefs(): void {
  if (agentDefinitions.length === 0) {
    console.log(dim("  No agent definitions loaded."));
    return;
  }

  for (const def of agentDefinitions) {
    console.log(dim(`  ${def.name} — ${def.description}`));
    console.log(dim(`    Tools: ${def.allowedTools.join(", ") || "all"}`));
    console.log(dim(`    Max turns: ${def.maxTurns}`));
  }
}
...
function startREPL(): void {
  ...
  console.log(bold("================================================"));
  console.log(bold("  Angel Agent — Phase 6: Polish & Extend"));
  console.log(dim(`  Tools: ${toolNames}, delegate_task`));
  console.log(dim("  Features: retry | fallback | hooks | plugins | MCP"));
  ...
  function prompt(): void {
    rl.question(green("You> "), async (input) => {
      ...
      if (trimmed.toLowerCase() === "/plugins") {
        handlePlugins();
        console.log("");
        prompt();
        return;
      ...
      if (trimmed.toLowerCase().startsWith("/logs")) {
        const args = trimmed.slice("/logs".length);
        handleLogs(args);
        console.log("");
        prompt();
        return;
      ...
      if (trimmed.toLowerCase() === "/agent-defs") {
        handleAgentDefs();
        console.log("");
        prompt();
        return;
...
async function main(): Promise<void> {
  ...
  if (useMCP) {
    ...
    console.log(dim(`  [mcp] Server started on port ${mcpPort}`));
